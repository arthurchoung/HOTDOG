...
@",\"Search Internet...\",\"1\"\n"
@",,\n"
@",\"Quit MacPlatinumDesktop\",\"'desktopObject'|valueForKey|exit:0\"\n"
;
...
+ (id)getMacPlatinumDirForPath:(id)path
{
    ...
    for (int i=0; i<[objectWindows count]; i++) {
        ...
        if (object) {
            ...
            if ([className isEqual:@"MacPlatinumDir"]) {
...
+ (void)openMacPlatinumDirForPath:(id)path
{
    ...
    id elt = [Definitions getMacPlatinumDirForPath:realPath];
    if (elt) {
    ...
    if ([realPath isDirectory]) {
        id object = [Definitions MacPlatinumDir:realPath];
        if (object) {
...
@implementation Definitions(fmekwlmfklsdmfklsmdklfmksldfjdksjffjdskfjsdkfjksk)
...
exit(1);
    ...
    id obj = [@"MacPlatinumDesktop" asInstance];
    ...
    for (int i=0; i<[lines count]; i++) {
        ...
        if (!dict) {
            ...
            if ([className length]) {
                ...
                obj = [@"MacPlatinumComputerIcon" asInstance];
            }
