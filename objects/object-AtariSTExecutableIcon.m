...
static char *executablePalette =
"b #000000\n"
". #ffffff\n"
;
static char *selectedExecutablePalette =
". #000000\n"
"b #ffffff\n"
;
static char *executablePixels =
...
- (int)preferredWidth
{
    static int w = 0;
    if (!w) {
        w = [Definitions widthForCString:executablePixels];
        if ([_path length]) {
...
- (int)preferredHeight
{
    static int h = 0;
    if (!h) {
        h = [Definitions heightForCString:executablePixels];
        h += 16;
    }
    return h;
}
...
- (void)drawInBitmap:(id)bitmap rect:(Int4)r context:(id)context
{
    ...
    int w = [Definitions widthForCString:executablePixels];
    int h = [Definitions heightForCString:executablePixels];

    if (hasFocus || isSelected) {
        [bitmap drawCString:executablePixels palette:selectedExecutablePalette x:r.x+(r.w-w)/2 y:r.y];
    } else {
        [bitmap drawCString:executablePixels palette:executablePalette x:r.x+(r.w-w)/2 y:r.y];
    }
    if (hasFocus || isSelected) {
