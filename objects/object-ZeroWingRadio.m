...
static void destructiveColorizeRadio(unsigned char *cstr)
{
...
@implementation Definitions(fjewmfnkdslnfsdjffdsjkflsdmkmklfdsjfjdskfjkjfkdsjfks)
+ (id)ZeroWingRadio
{
    id lines = [Definitions linesFromStandardInput];
    id obj = [@"ZeroWingRadio" asInstance];
...
- (id)init
{
    ...
    if (self) {
        ...
        pixels = nsfmt(@"%s", radio_pixels);
        destructiveColorizeRadio([pixels UTF8String]);
        [self setValue:pixels forKey:@"radioPixels"];
        pixels = nsfmt(@"%s", radio_selected_pixels);
        destructiveColorizeRadio([pixels UTF8String]);
        [self setValue:pixels forKey:@"radioSelectedPixels"];
        pixels = nsfmt(@"%s", radio_down_pixels);
        destructiveColorizeRadio([pixels UTF8String]);
        [self setValue:pixels forKey:@"radioDownPixels"];
    }
    return self;
}
- (BOOL)shouldAnimate
...
- (int)preferredHeight
{
    ...
    for (int i=0; i<[_array count]; i++) {
        ...
        text = [bitmap fitBitmapString:text width:w-radioWidth-10-18*4];
        h += [bitmap bitmapHeightForText:text] + lineHeight;
    }
    h += 21 + 28 + 21;
    return h;
...
- (void)drawInBitmap:(id)bitmap rect:(Int4)r
{
    ...
    int radioWidth = [Definitions widthForCString:[_radioPixels UTF8String]];
    int radioHeight = [Definitions heightForCString:[_radioPixels UTF8String]];
    for (int i=0; i<[_array count]; i++) {
        id elt = [_array nth:i];
        id text = [elt valueForKey:@"text"];
        if (!text) {
        ...
        _rect[i].x = x;
        _rect[i].y = y;
        text = [bitmap fitBitmapString:text width:r.w-radioWidth-10-18*4];
        int textWidth = [bitmap bitmapWidthForText:text];
        int textHeight = [bitmap bitmapHeightForText:text];
        _rect[i].w = radioWidth+10+textWidth;
        _rect[i].h = textHeight;
        char *str = [_radioPixels UTF8String];
        if ((_down==i+1) && (_hover==i+1)) {
            str = [_radioDownPixels UTF8String];
        }
        [bitmap drawCString:str palette:[_textPalette UTF8String] x:x y:y];
        if (_selectedIndex == i) {
            [bitmap drawCString:[_radioSelectedPixels UTF8String] palette:[_textPalette UTF8String] x:x y:y];
        }
        [bitmap drawBitmapText:text x:x+radioWidth+10 y:y+1 palette:_textPalette];
        y += textHeight + lineHeight;
    }
...
- (void)handleMouseDown:(id)event
{
    ...
    for (int i=0; i<[_array count]; i++) {
        if ([Definitions isX:mouseX y:mouseY insideRect:_rect[i]]) {
            if (mouseY < _minRadioY) {
                continue;
            }
            if (mouseY > _maxRadioY) {
                continue;
            }
            _down = i+1;
            _hover = i+1;
