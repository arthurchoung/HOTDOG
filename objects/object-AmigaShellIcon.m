...
"o #ffffff\n"
;
static char *selectedShellPalette =
"b #000000\n"
"o #000022\n"
"X #ff8800\n"
"* #0055aa\n"
...
@end
@implementation AmigaShellIcon
- (int)preferredWidth
{
    static int w = 0;
    if (!w) {
        w = [Definitions widthForCString:shellPixels];
        if ([_path length]) {
...
- (int)preferredHeight
{
    static int h = 0;
    if (!h) {
        h = [Definitions heightForCString:shellPixels];
        h += 16;
    }
    return h;
}
...
- (void)drawInBitmap:(id)bitmap rect:(Int4)r context:(id)context
{
    ...
    int w = [Definitions widthForCString:shellPixels];
    int h = [Definitions heightForCString:shellPixels];

    if (hasFocus || isSelected) {
        [bitmap drawCString:shellPixels palette:selectedShellPalette x:r.x+(r.w-w)/2 y:r.y];
    } else {
        [bitmap drawCString:shellPixels palette:shellPalette x:r.x+(r.w-w)/2 y:r.y];
    }
    if ([_path length]) {
