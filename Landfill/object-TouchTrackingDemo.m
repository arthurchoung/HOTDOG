...
{
    id obj = [@"TouchTrackingDemo" asInstance];
...
@interface TouchTrackingDemo : IvarObject
{
    double _puckX;
    double _puckY;
...
@end
@implementation TouchTrackingDemo
...
- (void)drawInBitmap:(id)bitmap rect:(Int4)r
{
    ...
    for (int i=0; i<MAX_SLOTS; i++) {
        ...
        double xPct = (double)(_x[i] - _minX) / (double)(_maxX - _minX);
        double yPct = (double)(_y[i] - _minY) / (double)(_maxY - _minY);
        int x = r.w*xPct;
        int y = r.h*yPct;
        [bitmap fillRectangleAtX:x-radius y:y-radius w:radius*2 h:radius*2];
    }
