...
@implementation Definitions(mekfowmkdlsmfklmsdklfmksld)
+ (id)TouchRubberBandDemo:(id)path
{
    id obj = [@"TouchRubberBandDemo" asInstance];
    if (![obj openTrackpad:path]) {
NSLog(@"Unable to open trackpad device '%@'", path);
        exit(1);
    }
    return obj;
}
...
@interface TouchRubberBandDemo : IvarObject
{
    double _objectOffsetY;
    double _maxObjectOffsetY;
...
@implementation TouchRubberBandDemo
...
- (void)drawInBitmap:(id)bitmap rect:(Int4)r
{
    ...
    int count = 100;
    double contentHeight = cellHeight * count;
    if (contentHeight < r.h) {
        contentHeight = r.h;
    }
    _maxObjectOffsetY = contentHeight - r.h;
    for (int i=0; i<count; i++) {
        ...
        [bitmap fillRectangleAtX:0 y:_objectOffsetY+(cellHeight*i) w:(r.w/count)*(i+1) h:cellHeight];
    }
}
@end
